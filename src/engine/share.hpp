...
namespace engine {
    class Share {
        public:
            ...
            std::vector<std::pair<std::uint16_t, std::uint64_t>> getBuyPricesAndQuantities(std::uint64_t quantityLimit = 0, std::uint16_t priceLimit = 1, bool inverted = false) const;
            std::vector<std::pair<std::uint16_t, std::uint64_t>> getSellPricesAndQuantities(std::uint64_t quantityLimit = 0, std::uint16_t priceLimit = MAX_DENOMINATIONS, bool inverted = false) const;
